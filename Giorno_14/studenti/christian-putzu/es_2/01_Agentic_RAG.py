...
try:
    ...
    for item in required_nltk_data:
        try:
            ...
            with st.spinner(f"Downloading required NLTK data ({item})..."):
...
class EntityEvaluator:
    """Classe per la valutazione delle performance del modello NER"""
    
    def __init__(self):
        self.true_entities = {}  # {doc_id: List[Dict]}
        self.pred_entities = {}  # {doc_id: List[Dict]}
        self.evaluation_history = []  # Store evaluation results history
        
    def add_document(self, doc_id: str, true_entities: List[Dict], pred_entities: List[Dict]):
        """Aggiunge un documento per la valutazione"""
        self.true_entities[doc_id] = true_entities
        self.pred_entities[doc_id] = pred_entities
    
    def _convert_to_bio_format(self, text: str, entities: List[Dict]) -> Tuple[List[str], List[str]]:
        """Converte le entità in formato BIO per la valutazione"""
        ...
        for entity in entities:
            ...
            for i, token in enumerate(tokens):
                ...
                if token_start >= start and token_end <= end:
                    labels[i] = f"B-{label}" if i == 0 or labels[i-1] == 'O' else f"I-{label}"
                token_start = token_end + 1
        
        return tokens, labels
    
    def evaluate_ner(self, doc_id: str, text: str) -> Dict:
        """Calcola metriche di valutazione NER per un documento"""
        true_tokens, true_labels = self._convert_to_bio_format(text, self.true_entities[doc_id])
        pred_tokens, pred_labels = self._convert_to_bio_format(text, self.pred_entities[doc_id])
        
        # Calculate metrics per entity type
        entity_types = set([label.split('-')[1] for label in true_labels + pred_labels if label != 'O'])
        metrics = {}
        
        for entity_type in entity_types:
            # Convert labels to binary for this entity type
            true_binary = [1 if label.endswith(entity_type) else 0 for label in true_labels]
            pred_binary = [1 if label.endswith(entity_type) else 0 for label in pred_labels]
    ...
    def plot_metrics(self, metrics: Dict) -> go.Figure:
        """Crea un grafico radar per le metriche di valutazione"""
        categories = []
        precision_values = []
        recall_values = []
        f1_values = []
        ...
        fig.update_layout(
            ...
            showlegend=True,
            title="Metriche per Tipo di Entità"
        )
        
        return fig
    
    def export_results(self, doc_id: str, metrics: Dict) -> Dict:
        """Esporta i risultati della valutazione in formato strutturato"""
        evaluation_result = {
            'document_id': doc_id,
            'timestamp': pd.Timestamp.now().isoformat(),
            'metrics': metrics,
            'true_entities_count': len(self.true_entities.get(doc_id, [])),
            'pred_entities_count': len(self.pred_entities.get(doc_id, [])),
            'entity_types': list(set(
                [e.get('entity_group') for e in self.true_entities.get(doc_id, [])] +
                [e.get('entity_group') for e in self.pred_entities.get(doc_id, [])]
            ))
        }
...
class TextEvaluator:
    """Classe per la valutazione delle risposte generate"""
    ...
    def evaluate_text(self, generated_text: str, reference_text: str) -> Dict:
        """Calcola metriche di valutazione del testo"""
        # Calcola BLEU
        ...
        # Calcola ROUGE
        rouge_scores = self.scorer.score(generated_text, reference_text)
        
        # Calcola BERTScore
        bert_scores = score([generated_text], [reference_text], lang="it")
        ...
        return {
            ...
            'bert_score': {
                'precision': float(bert_scores[0].mean()),
                'recall': float(bert_scores[1].mean()),
                'f1': float(bert_scores[2].mean())
            }
        }

    def generate_reference_text(self, client: AzureOpenAI, text: str) -> str:
        """Genera un testo di riferimento usando GPT-4"""
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Sei un assistente esperto che genera risposte di riferimento di alta qualità per valutare le performance di altri modelli. Genera una risposta completa, accurata e ben strutturata."
                    },
                    {
                        "role": "user",
                        "content": f"Genera una risposta di riferimento per questo testo:\n\n{text}"
                    }
                ],
                max_tokens=800,
                temperature=0.2
            ...
            return f"Errore nella generazione del testo di riferimento: {e}"

    def plot_metrics(self, metrics: Dict) -> go.Figure:
        """Crea un grafico a barre per le metriche di valutazione del testo"""
        categories = ['BLEU']
        values = [metrics['bleu']]
        
        # Add ROUGE scores
        for rouge_type, score in metrics['rouge'].items():
            categories.append(rouge_type.upper())
            values.append(score)
        
        # Add BERTScore metrics
        for bert_type, score in metrics['bert_score'].items():
            categories.append(f'BERTScore_{bert_type}')
            values.append(score)
        ...
        fig.update_layout(
            title='Metriche di Valutazione del Testo',
            yaxis_title='Score',
    ...
    def export_results(self, doc_id: str, metrics: Dict, generated_text: str, reference_text: str) -> Dict:
        """Esporta i risultati della valutazione in formato strutturato"""
        evaluation_result = {
            'document_id': doc_id,
            'timestamp': pd.Timestamp.now().isoformat(),
...
def init_session_state():
    ...
    if 'entity_evaluator' not in st.session_state:
        st.session_state.entity_evaluator = EntityEvaluator()
    if 'text_evaluator' not in st.session_state:
        st.session_state.text_evaluator = TextEvaluator()
...
AZURE_EMBEDDING_DEPLOYMENT_NAME=your_embedding_deployment_name
    ...
    with tab6:
        st.header("📈 Valutazione Performance NER")
        
        if not st.session_state.anonymized_docs:
            st.warning("⚠️ Anonimizza prima alcuni documenti nella tab 'Anonimizzazione'")
        else:
            st.subheader("🎯 Inserisci Entità di Riferimento")
            ...
            for filename, doc_data in st.session_state.anonymized_docs.items():
                with st.expander(f"📄 {filename}"):
                    ...
                    st.text_area(
                        "Testo Originale",
                        value=doc_data['original'],
                        height=150,
                        disabled=True,
                        key=f"eval_orig_{filename}"
                    )
                    
                    # Genera entità di riferimento con GPT-4
                    if st.button("🤖 Genera Entità di Riferimento", key=f"gen_ref_{filename}"):
                        with st.spinner("Generando entità di riferimento con GPT-4..."):
                            prompt = f"""
                            Analizza questo testo e identifica tutte le entità sensibili (nomi, luoghi, date, numeri, ecc.).
                            Restituisci le entità in formato JSON con questa struttura:
                            [
                                {{"start": int, "end": int, "entity_group": str, "text": str}},
                                ...
                            ]
                            ...
                            response = st.session_state.processor.client.chat.completions.create(
                                model=Config.DEPLOYMENT_NAME,
                                messages=[
                                    {"role": "system", "content": "Sei un esperto di NER che identifica entità sensibili nei testi."},
                            ...
                            try:
                                true_entities = json.loads(response.choices[0].message.content)
                                st.session_state.entity_evaluator.add_document(
                                    filename,
                                    true_entities,
                                    doc_data['entities']
                                )
                                st.success("✅ Entità di riferimento generate!")
                            except Exception as e:
                                st.error(f"Errore nel parsing delle entità: {e}")
                    
                    # Se abbiamo entità di riferimento, mostra metriche e grafico
                    if filename in st.session_state.entity_evaluator.true_entities:
                        st.subheader("📊 Metriche di Valutazione")
                        metrics = st.session_state.entity_evaluator.evaluate_ner(filename, doc_data['original'])
                        
                        # Mostra metriche per tipo di entità
                        for entity_type, scores in metrics.items():
                            if entity_type != 'overall':
                                st.write(f"**Tipo: {entity_type}**")
                                cols = st.columns(3)
                                cols[0].metric("Precision", f"{scores['precision']:.2%}")
                                cols[1].metric("Recall", f"{scores['recall']:.2%}")
                                cols[2].metric("F1", f"{scores['f1']:.2%}")
                        
                        # Mostra metriche complessive
                        st.write("**Metriche Complessive**")
                        cols = st.columns(3)
                        cols[0].metric("Precision", f"{metrics['overall']['precision']:.2%}")
                        cols[1].metric("Recall", f"{metrics['overall']['recall']:.2%}")
                        cols[2].metric("F1", f"{metrics['overall']['f1']:.2%}")
                        
                        # Visualizza grafico radar
                        st.plotly_chart(st.session_state.entity_evaluator.plot_metrics(metrics))
                        
                        # Esporta risultati
                        evaluation_result = st.session_state.entity_evaluator.export_results(filename, metrics)
                        st.download_button(
                            "📥 Scarica Risultati Valutazione NER",
                            data=json.dumps(evaluation_result, indent=2),
                            file_name=f"ner_evaluation_{filename}.json",
                            mime="application/json"
                        )

    with tab7:
        st.header("📊 Valutazione Generazione Testo")
        
        if not st.session_state.processed_docs:
            st.warning("⚠️ Analizza prima alcuni documenti nella tab 'Analisi'")
        else:
            st.subheader("🎯 Valutazione Risposte Generate")
            ...
            for filename, result in st.session_state.processed_docs.items():
                with st.expander(f"📄 {filename}"):
                    ...
                    st.text_area(
                        ...
                        height=150,
                        disabled=True,
                        key=f"gen_text_{filename}"
                    )
                    
                    # Genera testo di riferimento con GPT-4
                    if st.button("🤖 Genera Risposta di Riferimento", key=f"gen_ref_text_{filename}"):
                        with st.spinner("Generando risposta di riferimento con GPT-4..."):
                            reference_text = st.session_state.text_evaluator.generate_reference_text(
                                st.session_state.processor.client,
                                result['anonymized_text']
                            )
                            st.session_state.reference_texts[filename] = reference_text
                            st.success("✅ Risposta di riferimento generata!")
                    
                    # Se abbiamo un testo di riferimento, mostra e calcola metriche
                    if filename in st.session_state.reference_texts:
                        st.write("**Risposta di Riferimento:**")
                        st.text_area(
                            "Testo di riferimento",
                            value=st.session_state.reference_texts[filename],
                            height=150,
                            disabled=True,
                            key=f"ref_text_{filename}"
                        )
                        
                        # Calcola e mostra metriche
                        metrics = st.session_state.text_evaluator.evaluate_text(
                            result['analysis'],
                            st.session_state.reference_texts[filename]
                        )
                        ...
                        # BLEU
                        st.write("**BLEU Score:**")
                        st.metric("BLEU", f"{metrics['bleu']:.2%}")
                        
                        # ROUGE
                        st.write("**ROUGE Scores:**")
                        rouge_cols = st.columns(3)
                        rouge_cols[0].metric("ROUGE-1", f"{metrics['rouge']['rouge1']:.2%}")
                        rouge_cols[1].metric("ROUGE-2", f"{metrics['rouge']['rouge2']:.2%}")
                        rouge_cols[2].metric("ROUGE-L", f"{metrics['rouge']['rougeL']:.2%}")
                        
                        # BERTScore
                        st.write("**BERTScore:**")
                        bert_cols = st.columns(3)
                        bert_cols[0].metric("Precision", f"{metrics['bert_score']['precision']:.2%}")
                        bert_cols[1].metric("Recall", f"{metrics['bert_score']['recall']:.2%}")
                        bert_cols[2].metric("F1", f"{metrics['bert_score']['f1']:.2%}")

                        # Visualizza grafico a barre
                        st.plotly_chart(st.session_state.text_evaluator.plot_metrics(metrics))
                        
                        # Esporta risultati
                        evaluation_result = st.session_state.text_evaluator.export_results(
                            filename,
                            metrics,
                            result['analysis'],
                            st.session_state.reference_texts[filename]
                        ...
                        st.download_button(
                            "📥 Scarica Risultati Valutazione Testo",
                            data=json.dumps(evaluation_result, indent=2),
                            file_name=f"text_evaluation_{filename}.json",
                            mime="application/json"
                        )
