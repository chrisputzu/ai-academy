...
def init_semantic_state():
    """Inizializza stato per analisi semantica"""
    if 'semantic_evaluator' not in st.session_state:
        from semantic_evaluator import SemanticBiasEvaluator
        st.session_state.semantic_evaluator = SemanticBiasEvaluator(
            azure_client=st.session_state.azure_client if 'azure_client' in st.session_state else None
        )
    
    if 'semantic_query' not in st.session_state:
        st.session_state.semantic_query = ""
    
    if 'semantic_response' not in st.session_state:
        st.session_state.semantic_response = ""

def run_semantic_analysis(query: str, response: str) -> Dict:
    """Esegue analisi semantica"""
    if not query or not response:
        st.warning("⚠️ Inserisci sia la domanda che la risposta")
        return None
    
    with st.spinner("🧠 Analisi semantica in corso..."):
        evaluation = st.session_state.semantic_evaluator.evaluate_response_semantically(
            query, response
        )
        return evaluation

def clear_semantic_state():
    """Pulisce stato analisi semantica"""
    if 'semantic_evaluator' in st.session_state:
        st.session_state.semantic_evaluator.evaluations = []
